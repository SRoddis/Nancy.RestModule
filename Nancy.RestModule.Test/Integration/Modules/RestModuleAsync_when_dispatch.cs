...
{
    [TestFixture]
    public class RestModuleAsync_when_dispatch
    {
        [SetUp]
        ...
        {
            // Act
            BrowserResponse post = _browser.Delete("/testAsync/1", with => with.HttpRequest());
        ...
        {
            // Act
            BrowserResponse response = _browser.Get("/testAsync", with => with.HttpRequest());
        ...
        {
            // Act
            BrowserResponse post = _browser.Post("/testAsync", with =>
            {
                with.HttpRequest();
                with.Query("value", "3");
            });
